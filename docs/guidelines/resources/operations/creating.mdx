...
* `POST` is used to create a new resource under a collection of such resources[^1], whereas `PUT` is used to create or update a resource with a specific identifier[^2] (under premise that the client is allowed to choose the identifier)
