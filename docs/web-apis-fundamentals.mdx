...
:::tip
Read [here](/docs/guidelines/advanced/rest-alternatives) for more information on how REST APIs stack up against other HTTP APIs
:::

As it can be seen on the image below, the only requirement that REST API imposes is reliance on HTTP.
