...
REST APIs are stateless, meaning the server doesn't store any client state (in the instance's memory, local file system, etc.). This way there's no need for any kind of session stickiness.
This means that any server instance where the request lands can fulfill the request. This also means that the server can increase the performance by adding more instances, and load balancing between them.
